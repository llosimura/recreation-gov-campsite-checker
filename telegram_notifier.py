...
CREDENTIALS_FILE = "./telegram_config.json"


def _send_telegram_msg(message):
    with open(CREDENTIALS_FILE) as f:
        tc = json.load(f)
        url = f"https://api.telegram.org/bot{tc['token']}/sendMessage?chat_id={tc['chat_id']}&text={message}"
...
def main(args, stdin):
    ...
    if "Something went wrong" in first_line:
        _send_telegram_msg("{}, I'm broken! Please help")
        sys.exit()
